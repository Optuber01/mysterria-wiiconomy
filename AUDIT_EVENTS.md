...
operation has a UUID `correlationId` and a separate stable string `businessId`. Every result,
failure, refund, courier handoff, and recovery step for that operation reuses both values.
...
| `agora.listing.failed`, `agora.listing.fee_refunded` | Listing validation/charge/insert failure | price, fee, reason |
...
| `stash.claimed` | Stash claim batch result | delivered, remaining |
| `ledger.claimed`, `ledger.claim_failed`, `ledger.claim_recovered`, `ledger.claim_reverted` | Proceeds deposit/claim/recovery result | original claim identity, monetary projection |
| `plots.rent.committed`, `plots.rent.failed` | Plot rent charge and DB claim | monetary projection, plot_id, paid_until, reason |
| `plots.rent.upkeep`, `plots.rent.upkeep_failed` | Plot extension charge and DB update | monetary projection, plot_id, paid_until |
| `plots.rent.refunded` | Failed rent/upkeep refund | original rent/upkeep identity, monetary projection, reason |
...
| `plot_shop.ledger_failed`, `plot_shop.refunded` | Stall ledger failure/refund | purchase identity, monetary projection, plot_id, net, reason |
